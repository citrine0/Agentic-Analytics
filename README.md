...
*   *为两种不同数据类型选择对应的agent，其中python agent负责处理csv数据，生成对应的Markdown表格和图片，sql agent负责处理db数据，生成对应的表格数据。*

#### 2.2.2 自研线性执行器：从“黑盒失控”到“白盒可控”
*   *我们相信，最好的 Agent 是最简单的 Agent。因此，我们构建了一个极简的线性执行器，它剥离了所有不必要的复杂性，让 AI 的核心能力——代码生成——直接、高效地服务于数据分析任务。*

### 2.3 交互优化：聚焦单次任务的流畅体验
*   *我们通过一个巧妙的“两步刷新”逻辑来优化交互体验：用户输入后立即进行第一次 rerun 以锁定其消息，待AI处理完毕后，第二次 rerun 再将助手的完整回复优雅地追加到对话流中，确保了界面的流畅与稳定。*
...
## 6. 思考与洞察：关于AIGC产品稳定性的探索

在本项目V1的实现与测试过程中，我遇到了一个非常典型且深刻的AIGC产品挑战：即使在temperature=0的确定性设置下，对于完全相同的输入，LLM在不同时间的输出依然会产生不一致，甚至导致功能失败。让我深刻认识到，构建一个可靠的AIGC产品，绝不能仅仅依赖于上游模型的理想表现。我们必须在产品架构中，内建一个强大的“稳定保障体系”。

基于此，我为产品的下一阶段（V1.5）规划了如下的多层次、防御性的迭代方案：
1. 宏观原则层 (System Prompt as "Constitution"):
问题: 过于具体的规则会导致Prompt臃肿且不具备普适性。
解决方案: 将System Prompt重新定位为设定“宪法”级别的宏观原则，如**代码安全（禁止文件/网络IO）、输出格式规范、以及代码风格（PEP 8）**等，而将具体函数的处理交给后续流程。

2. 事中校验层 (Pre-execution Validation):
问题: 大量错误可以在代码实际运行前被发现。
解决方案: 在执行LLM生成的代码前，引入两道“安检”：
静态分析 (Linting): 通过ast库解析代码，拦截明显的语法错误。
安全扫描 (Security Scan): 规则化地扫描并禁止已知的危险函数调用。

4. 事后容错与修复层 (Post-execution Fault Tolerance):
问题: 对于未知的、运行时才会暴露的逻辑错误，需要有强大的兜底机制。
解决方案: 设计一个“**AI自我纠错**”的循环：
将代码执行器包裹在try...except中。
一旦捕获到异常（如TypeError），系统会自动将**原始指令、错误代码、以及完整的错误信息（Traceback）**打包成一个新的“修复式Prompt”。
将此Prompt再次发送给LLM，请求其进行自我修复，从而实现一个弹性的、高鲁棒性的系统。
...
## 8. AI辅助开发与研究 (AI-Assisted Development & Research)
